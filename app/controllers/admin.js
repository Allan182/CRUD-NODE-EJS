...
module.exports.atualizarNoticia = function (app, req, res) {
    ...
    var erros = req.validationErrors();
    if (erros) {
        res.render("admin/form_editar_noticia", { validacao: erros, noticia: [noticia], flagAdmin: req.session.autorizado });
        return;
    }
    var connection = app.config.dbConnection();
    var noticiaModel = new app.app.models.manchetesDAO(connection);

    noticiaModel.updateNoticia(noticia, function (error, result) {
        res.redirect('/principaisnoticias');
    });
}
...
module.exports.form_editar = function (app, req, res) {
    ...
    noticiaModel.getNoticiaUpdate(id, function (error, result) {
        res.render("admin/form_editar_noticia", { validacao: {}, noticia: result, flagAdmin: req.session.autorizado });
    });
}